...
public class ScriptTests {
    ...
    public ScriptTests() {
        ...
        kit = new WalletAppKit(networkParameters, new File(wallet_name), "password1");
    }
    ...
    private void testTransaction(ScriptTransaction scriptTransaction) throws InsufficientMoneyException {
        ...
        Transaction transaction = scriptTransaction.createOutgoingTransaction(inputScript, Coin.CENT.multiply(10));
        //Transaction transaction = scriptTransaction.createOutgoingTransaction(inputScript, Coin.MILLICOIN.divide(2));
...
//        try (ScriptTransaction payToPubKey = new PayToPubKey(networkParameters, new File(wallet_name), "password1")) {
...
//        try (ScriptTransaction payToPubKeyHash = new PayToPubKeyHash(networkParameters, new File(wallet_name), "password1")) {
...
//        try (LinearEquationTransaction linEq = new LinearEquationTransaction(networkParameters, new File(wallet_name), "password1")) {
...
//        try (ScriptTransaction multiSig = new MultiSigTransaction(networkParameters, new File(wallet_name), "password1")) {
